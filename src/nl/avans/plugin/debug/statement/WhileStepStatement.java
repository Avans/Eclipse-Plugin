...
public class WhileStepStatement extends StepStatement {
	...
	public WhileStepStatement(WhileStatement statement, IType type) {
		...
		this.expression = EvaluatableExpression.getEvaluatableExpressionForExpression(statement.getExpression());
	}
	...
	public Step createStepFromThread(IJavaStackFrame stackframe) {
		...
		for(int line = this.line; line <= endLine; line++) {
			if(line == this.line)
				step.stepLines.add(new StepLine("Omdat " + expression.evaluateForPresentableString(stackframe) + " waar is...", line, true));
