...
public class InFixEvaluatableExpression extends EvaluatableExpression {
	...
	public InFixEvaluatableExpression(InfixExpression expression) {
		super(expression);
		left_hand = getEvaluatableExpressionForExpression(expression.getLeftOperand());
		right_hand = getEvaluatableExpressionForExpression(expression.getRightOperand());
		operator = expression.getOperator().toString();
	}

	@Override
	public String toEvaluatableString() {
		return left_hand.toEvaluatableString() + " " + operator + " " + right_hand.toEvaluatableString();
	}
